...
figure();
%--------------
%güç spektral yoðunluk fonksiyonu (pwelch) kullanarak hesapladýk
window = round(length(s)/4);
noverlap = window/2;
freq = 0:0.01:5;
fs = 100; % Hz örnekleme aralýðý aldýk
[pxx,f] = pwelch(s,window,noverlap,freq,fs);
title('güç spektral yoðunluðu');
plot(f,pxx)

%kalbe elektriksel uyarý ; P Dalgasý
% ardýndan kalbin atýmý ile  Q R S T komplesi gözlemlenir.
%kalbin ortasýna elektrotlar yerleþtirildikten sonra 1-8kHz kadar kayýt
%alýnabilmektedir
...
RR = diff(locs);    %n.konumdan n-1. konumu çýkar
figure
plot(RR,'ro')
